...
func _apply_menu_hotspots(main: Control) -> void:
	...
	if bg == null or bg.texture == null:
		return
	var source_size := Vector2(bg.texture.get_size())
	if source_size.x <= 0.0 or source_size.y <= 0.0 or bg.size.x <= 0.0 or bg.size.y <= 0.0:
		return
	var sx := bg.size.x / source_size.x
	var sy := bg.size.y / source_size.y
	for child in main.get_children():
...
func _hide_inline_inventory(main: Control) -> void:
	...
	for child in main.get_children():
		if child is PanelContainer:
			...
			var is_inventory := panel.position.x <= 30.0 and panel.position.y >= view_height - 190.0 and panel.size.x <= 210.0
			if is_inventory:
				panel.visible = false
...
func _open_inventory() -> void:
	...
	var hint := Label.new()
	hint.text = "Selecione um item e depois clique no alvo do cenário."
